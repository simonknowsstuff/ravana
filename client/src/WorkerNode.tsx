...
interface TextureData { width: number; height: number; pixels: Uint8Array; }

interface GeometryCache {
  positions: Float32Array; indices: Uint32Array; bvhBuffer: Float32Array;
  colors: Float32Array; normals: Float32Array; emissive: Float32Array; ao: Float32Array;
  uvs: Float32Array; textureIndices: Float32Array; textures: TextureData[];
  emissiveTextureIndices: Float32Array; transmissionArray: Float32Array; iorArray: Float32Array;
...
export default function WorkerNode() {
  ...
  useEffect(() => {
    const worker = new RaytracerWorker(); workerRef.current = worker;
    ...
    socket.on('connect', () => { setIsConnected(true); setStatus('Connected. Awaiting payload...'); socket.emit('register_worker'); });
    socket.on('disconnect', () => { setIsConnected(false); setStatus('Lost connection to Swarm.'); });
    socket.on('error', (err) => { setErrorLog(`Socket error: ${err}`); });
    socket.on('connect_error', (err) => { setErrorLog(`Connection error: ${err}`); });
    ...
    socket.on('sync_geometry', (payload) => {
      try {
        ...
        const transmissionArray = m.transmissionLength > 0 ? new Float32Array(rawBuffer.slice(m.transmissionOffset, m.transmissionOffset + m.transmissionLength)) : new Float32Array(0);
        const iorArray = m.iorLength > 0 ? new Float32Array(rawBuffer.slice(m.iorOffset, m.iorOffset + m.iorLength)) : new Float32Array(0);
        
        // ── UNPACK VOLUME ──
        const attenuationColorArray = m.attenuationColorLength > 0 ? new Float32Array(rawBuffer.slice(m.attenuationColorOffset, m.attenuationColorOffset + m.attenuationColorLength)) : new Float32Array(0);
        const attenuationDistanceArray = m.attenuationDistanceLength > 0 ? new Float32Array(rawBuffer.slice(m.attenuationDistanceOffset, m.attenuationDistanceOffset + m.attenuationDistanceLength)) : new Float32Array(0);

        const textures: TextureData[] = (m.textures || []).map((t: any) => ({ width: t.width, height: t.height, pixels: new Uint8Array(rawBuffer.slice(t.pixelsOffset, t.pixelsOffset + t.pixelsLength)) }));

        geoCacheRef.current = { positions, indices, bvhBuffer, colors, normals, emissive, ao, uvs, textureIndices, textures, emissiveTextureIndices, transmissionArray, iorArray, attenuationColorArray, attenuationDistanceArray };
        ...
        workerRef.current?.postMessage({
          ...
          uvs, textureIndices, textures, emissiveTextureIndices,
          transmissionArray, iorArray, attenuationColorArray, attenuationDistanceArray // ── ADD TO WORKER PAYLOAD
        });

        if (pendingTaskRef.current) {
          const queued = pendingTaskRef.current;
          workerRef.current?.postMessage({ type: 'render_tile', startX: queued.startX, startY: queued.startY, width: queued.width, height: queued.height, canvasWidth: queued.canvasWidth, canvasHeight: queued.canvasHeight, camera: queued.camera, sunDir: queued.sunDir, lights: queued.lights, });
          pendingTaskRef.current = null;
        }

      } catch (err: any) { setErrorLog(`Unpack Error: ${err.message}`); setStatus('Error during unpacking.'); }
    });

    socket.on('assign_tile', (task) => {
      if (!geoCacheRef.current) { pendingTaskRef.current = task; return; }
      setStatus(`Crunching tile [${task.startX}, ${task.startY}]...`);
      workerRef.current?.postMessage({ type: 'render_tile', startX: task.startX, startY: task.startY, width: task.width, height: task.height, canvasWidth: task.canvasWidth, canvasHeight: task.canvasHeight, camera: task.camera, sunDir: task.sunDir, lights: task.lights, });
    });

    workerRef.current.onmessage = (event) => {
      const { buffer, startX, startY, width, height } = event.data;
      socket.emit('tile_finished', { buffer, startX, startY, width, height });
      setTilesProcessed((prev) => prev + 1); setStatus('Tile complete. Requesting next...');
    };

    workerRef.current.onerror = (error) => { setErrorLog(`Web Worker crashed: ${error.message}`); setStatus('ERROR: Web Worker crashed'); };
    return () => { socket.disconnect(); workerRef.current?.terminate(); };
  }, []);
  ...
  return (
    <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center font-sans">
      ...
      {errorLog && <p className="text-red-400 font-mono text-xs mb-6 max-w-sm bg-red-900/30 p-2 rounded border border-red-700">{errorLog}</p>}
      <div className="bg-slate-800 rounded-2xl p-8 w-full max-w-sm border border-slate-700 shadow-xl relative overflow-hidden mt-4">
        ...
        <div className="flex items-baseline justify-center space-x-2">
          <p className="text-7xl font-black text-white">{tilesProcessed}</p><span className="text-xl text-slate-500 font-medium">tiles</span>
        </div>
      </div>
